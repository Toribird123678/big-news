...
if (isset($_POST['insertArticleBtn'])) {
	...
	$newId = $articleObj->createArticle($title, $description, $author_id);
	if ($newId) {
...
if (isset($_POST['editArticleBtn'])) {
	...
	if ($articleObj->canUserEditArticle($article_id, $_SESSION['user_id']) && $articleObj->updateArticle($article_id, $title, $description)) {
